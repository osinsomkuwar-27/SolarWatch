# Product Requirements Document: Aditya-L1 Solar Flare Prediction Pipeline

## Project Goal

Build and maintain a modular, end-to-end machine learning pipeline that ingests raw FITS data from ISRO's Aditya-L1 SoLEXS and HEL1OS payloads and produces:

- Clean, structured, multi-day time series data for both instruments
- Engineered features suitable for flare prediction modeling
- Detected flare events derived from instrument data
- A labeled, ML-ready dataset
- Trained and evaluated flare prediction models
- A backend integration layer capable of serving predictions and pipeline outputs
- A frontend application that presents data and predictions to end users

The pipeline must be reproducible, modular (each stage independently runnable and testable), and extensible to future instruments, models, backend endpoints, and frontend features without requiring redesign of existing modules.
...
- The system must support exploratory data analysis specific to SoLEXS soft X-ray measurements.
...
- The system must report evaluation results in a structured, inspectable format.
...
- The backend must be configurable (model artifact paths, dataset paths, serving parameters) through the centralized configuration module rather than hardcoded values.

### Frontend Requirements

- The frontend must allow users to view SoLEXS and HEL1OS light curves and associated exploratory data analysis outputs.
- The frontend must allow users to view detected flare events in relation to instrument time series.
...
- The frontend must be developed and reviewed on its own branch (`shreeja/frontend`) until ready for integration into `main`, following the same modularity principle as the rest of the pipeline.
...
- The frontend must be capable of rendering the same categories of visualizations (light curves, flare events, predictions) for end users, sourced from backend-provided data rather than duplicating pipeline computation in the frontend.
...
- The system must support generating evaluation reports summarizing model performance on test data.
- Reports must reference the dataset version and model artifact used, to ensure traceability.
- The system must avoid embedding unverified or placeholder metrics in generated reports.

### Configuration Requirements

- The system must centralize configuration (paths, parameters, instrument settings, backend serving settings) in the `config/` module rather than scattering configuration across individual scripts.
...
- Deployment configuration (e.g., model artifact location, serving parameters, frontend API base URL) must be managed through configuration rather than hardcoded into backend or frontend code.
...
- The `shreeja/frontend` branch must be mergeable into `main` without requiring structural changes to `backend/`, `config/`, or `ml/`, beyond well-defined API integration points.

### Future Enhancements

- Merge and stabilize the frontend currently under development on `shreeja/frontend`.
...
This document describes functional and non-functional requirements for the existing and ongoing development of the Aditya-L1 SoLEXS/HEL1OS solar flare prediction pipeline, including its backend integration layer and its in-development React frontend (`shreeja/frontend` branch). It does not introduce new modules beyond those already present or planned in the repository (configuration, utilities, loaders, FITS parsing, caching, SoLEXS EDA, HEL1OS EDA, cross-instrument analysis, flare detection, feature engineering, dataset generation, model training, evaluation, inference, backend integration, and the frontend application).